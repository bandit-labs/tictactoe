...
@router.post("/export")
def export_dataset(request: DatasetExportRequest, db=Depends(get_db)):
    """
    ...
    try:
        if request.format == "parquet":
            ...
            df = export_service.export_to_dataframe(
                max_games=request.max_games, mode_filter=request.mode_filter
            )
            df.to_csv(temp_path, index=False)
            stats = {"total_moves": len(df), "total_games": df["game_id"].nunique()}
        ...
        return FileResponse(
            ...
            headers={"X-Dataset-Stats": str(stats)},
        )
...
def get_dataset_stats(db=Depends(get_db)):
    ...
    total_games = (
        db.query(GameAnalytics)
        .filter(GameAnalytics.status.in_(["X_win", "O_win", "draw"]))
        .count()
    )

    outcomes = (
        db.query(GameAnalytics.status, func.count(GameAnalytics.game_id))
        .filter(GameAnalytics.status.in_(["X_win", "O_win", "draw"]))
        .group_by(GameAnalytics.status)
        .all()
    )
