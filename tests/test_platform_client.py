...
def test_log_move_to_platform_success(caplog):
    ...
    with patch("app.services.platform_client.requests.post") as mock_post:
        ...
        platform_client.log_move_to_platform(
            previous_state, new_state, move_index, player_id, heuristic_value
        )
        ...
        assert any(
            "Logged move to platform backend" in record.message
            and "game123" in record.message
...
def test_log_move_to_platform_failure(caplog):
    ...
    with patch("app.services.platform_client.requests.post") as mock_post:
        ...
        assert any(
            ...
            and "Network Error" in record.message
            for record in caplog.records
        )
...
def test_send_final_result_to_platform_success(caplog):
    ...
    with patch("app.services.platform_client.requests.post") as mock_post:
        ...
        # Check for successful log message
        assert any(
            "Sent final result to platform backend" in record.message
            and "game123" in record.message
            for record in caplog.records
        )
...
def test_send_final_result_to_platform_failure(caplog):
    ...
    with patch("app.services.platform_client.requests.post") as mock_post:
        ...
        # Check for error log message
        assert any(
            "Failed to send final result to platform" in record.message
            and "Server Error" in record.message
            for record in caplog.records
        )
