...
def test_state_to_game_finished_sets_finished_at():
    ...
    assert updated_orm_game.finished_at >= datetime(
        2023, 1, 1
    )  # Should be now or later than created_at
    ...
    updated_orm_game_with_finished_at = game_state_mapper.state_to_game(
        orm_game_with_finished_at, domain_state_won
    )
